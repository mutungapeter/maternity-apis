...
// register user
export default interface IRegistrationBody {
  name: string;
  email: string;
  password: string;
  role:string;
...
export const getUserInfo = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const user_id = req.user?._id || "";
      // const userId = (user_id as mongoose.Types.ObjectId).toString();
      getUserById(user_id as string, res);
    } catch (error: any) {
      return next(new ErrorHandler(error.message, 400));
    }
  }
...
console.log(oldPassword, newPassword)
      ...
      const user_id = req.user?._id || "";
      // const userId = (user_id as mongoose.Types.ObjectId).toString();
      await redis.set(user_id as string, JSON.stringify(user));
...
export const updateProfilePicture = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const  user  = await userModel.findById(user_id as string);
      if (avatar && user) {
        // if user have an avatar then call this
        if (user?.avatar?.public_id){
          //first deleter old image
